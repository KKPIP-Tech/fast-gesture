...
from fastgesture.data.new_datasets import TrainDatasets
from fastgesture.data.new_val_datasets import ValDatasets
...
class Train:
    def __init__(self, option, save_path:str, checkpoints=None) -> None:
        ...
        self._view:bool = True  # Demonstrate the training process
        ...
        # init Evaler -------------------------------------
        self._evaler:Evaler = Evaler(model=self._model, dataloader=self._val_dataloader, device=self._device, tensorboard_writer=self._writer)
        ...
        self._scheduler = optim.lr_scheduler.StepLR(self._optimizer, step_size=40, gamma=0.98)
        ...
        if checkpoints is not None:
            ...
            self._scheduler = optim.lr_scheduler.StepLR(self._optimizer, step_size=20, gamma=0.8, last_epoch=self._start_epoch-1)
    ...
    def train(self) -> None:
        for epoch in range(self._start_epoch, self._max_epoch):
            ...
            for index, datapack in enumerate(pbar):
                ...
                forward_heatmaps, forward_asf_maps = forward.heatmaps, forward.ascription_field
    ...
    def _loss_compute(self, forward_heatmaps, label_heatmaps, forward_asf_maps, label_asf_maps) -> float:
        ...
        for keypoints_index in range(self._keypoints_classes_num):
            if self._view:
                image_to_show = forward_heatmaps.permute(1, 0, 2, 3)[0][keypoints_index].cpu().detach().numpy().astype(np.float32)
        ...
        if self._view:
            image_to_show = label_asf_maps.permute(1, 0, 2, 3)[0][0].cpu().detach().numpy()  
            cv2.imshow(f"asf_label x", image_to_show)

            image_to_show = forward_asf_maps.permute(1, 0, 2, 3)[0][0].cpu().detach().numpy()  
        ...
        if self._view:
            image_to_show = label_asf_maps.permute(1, 0, 2, 3)[0][12].cpu().detach().numpy()  
            cv2.imshow(f"asf_label y", image_to_show)

            image_to_show = forward_asf_maps.permute(1, 0, 2, 3)[0][12].cpu().detach().numpy()  
            cv2.imshow(f"forward asf y", image_to_show)
        
        # comput x minus loss -----------------------------
        x_minus_loss = self._criterion_x_minus(forward_asf_maps[-2], label_asf_maps[-2])
        if self._view:
            image_to_show = label_asf_maps.permute(1, 0, 2, 3)[0][-2].cpu().detach().numpy() 
            cv2.imshow(f"asf_x_minus_label", image_to_show)
            image_to_show = forward_asf_maps.permute(1, 0, 2, 3)[0][-2].cpu().detach().numpy() 
            cv2.imshow(f"forward asf_x_minus", image_to_show)
        
        # comput y minus loss -----------------------------
        y_minus_loss = self._criterion_y_minus(forward_asf_maps[-1], label_asf_maps[-1])
        if self._view:
            image_to_show = label_asf_maps.permute(1, 0, 2, 3)[0][-1].cpu().detach().numpy() 
            cv2.imshow(f"asf_y_minus_label", image_to_show)
            image_to_show = forward_asf_maps.permute(1, 0, 2, 3)[0][-1].cpu().detach().numpy() 
            cv2.imshow(f"forward asf_y_minus", image_to_show)
            cv2.waitKey(1)
        
        loss = keypoints_loss*1.5 + asf_x_loss + asf_y_loss + x_minus_loss + y_minus_loss
        return loss
...
if __name__ == "__main__":
    ...
    parse.add_argument('--data', type=str,default='./data/new_config.yaml', help='training config path')
    parse.add_argument('--val', type=str,default='./data/new_config.yaml', help='val set config path')
    ...
    parse.add_argument('--val_batch', type=int, default=1, help='batch size')
